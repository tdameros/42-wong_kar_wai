...
/*   Updated: 2024/04/28 16:41:03 by bwisniew         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void print_win_menu(t_engine *engine, int8_t selected) {
	...
	size_x = (width / 2 > height ? height - MENU_MARGIN * 2: width / 2 - MENU_MARGIN * 2) * 2;
	...
	if (selected == BUTTON1)
		mvprintw(y_start + size_y - 5, x_start + ((size_x - 14) / 2), "» Continue «");
	else
		mvprintw(y_start + size_y - 5, x_start + ((size_x - 14) / 2), "  Continue  ");
	if (selected == BUTTON2)
	...
	mvprintw(0, 0, "height = %d, width = %d, size_x = %d, size_y = %d", x_start + ((size_x - 8) / 2), y_start + 5, size_x, size_y);
...
static void start_menu_callback(t_engine *engine, int32_t key) {
      if (key == KEY_UP) {
        ...
        engine->grid_size = ((engine->grid_size - 1) % MAX_GRID_SIZE) + MIN_GRID_SIZE;
...
static void print_start_menu(t_engine *engine, int8_t selected) {
	...
	if (selected == BUTTON2)
		...
		mvprintw(y_start + size_y - 7, x_start + ((size_x - 11) / 2), "  Score  ");
	if (selected == BUTTON3)
		mvprintw(y_start + size_y - 5, x_start + ((size_x - 19) / 2), "» Grid size : %d «", engine->grid_size);
	else
		mvprintw(y_start + size_y - 5, x_start + ((size_x - 19) / 2), "  Grid size : %d  ",  engine->grid_size);
	if (selected == BUTTON4)
		mvprintw(y_start + size_y - 3, x_start + ((size_x - 10) / 2), "» Exit «");
...
static void print_start_ascii(uint32_t x, uint32_t y)
{
    mvprintw(y, x, "____/\\\\\\\\\\\\\\\\\\__________/\\\\\\\\\\\\\\________________/\\\\\\_________/\\\\\\\\\\\\\\\\\\____        ");
    mvprintw(y + 1, x, " __/\\\\\\///////\\\\\\______/\\\\\\/////\\\\\\____________/\\\\\\\\\\_______/\\\\\\///////\\\\\\__       ");
    mvprintw(y + 2, x, "  _\\///______\\//\\\\\\____/\\\\\\____\\//\\\\\\_________/\\\\\\/\\\\\\______\\/\\\\\\_____\\/\\\\\\__      ");
    mvprintw(y + 3, x, "   ___________/\\\\\\/____\\/\\\\\\_____\\/\\\\\\_______/\\\\\\/\\/\\\\\\______\\///\\\\\\\\\\\\\\\\\\/___     ");
    mvprintw(y + 4, x, "    ________/\\\\\\//______\\/\\\\\\_____\\/\\\\\\_____/\\\\\\/__\\/\\\\\\_______/\\\\\\///////\\\\\\__    ");
    mvprintw(y + 5, x, "     _____/\\\\\\//_________\\/\\\\\\_____\\/\\\\\\___/\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\___/\\\\\\______\\//\\\\\\_   ");
    mvprintw(y + 6, x, "      ___/\\\\\\/____________\\//\\\\\\____/\\\\\\___\\///////////\\\\\\//___\\//\\\\\\______/\\\\\\__  ");
    mvprintw(y + 7, x, "       __/\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\___\\///\\\\\\\\\\\\\\/______________\\/\\\\\\______\\///\\\\\\\\\\\\\\\\\\/___ ");
    mvprintw(y + 8, x, "        _\\///////////////______\\///////________________\\///_________\\/////////_____");
...
static void print_loose_menu(t_engine *engine, int8_t selected) {
	...
	mvprintw(y_start + size_y - 5, x_start + ((size_x - 8) / 2), "Score : 0");
...
static void print_loose_ascii(uint32_t x, uint32_t y)
{
    ...
    mvprintw(y + 1, x, " \\ \\ / /___   _   _  | |     ___    ___   ___   ___ | |");
    mvprintw(y + 2, x, "  \\ V // _ \\ | | | | | |    / _ \\  / _ \\ / __| / _ \\| |");
    mvprintw(y + 3, x, "   | || (_) || |_| | | |___| (_) || (_) |\\__ \\|  __/|_|");
    mvprintw(y + 4, x, "   |_| \\___/  \\__,_| |_____|\\___/  \\___/ |___/ \\___|(_)");
}
